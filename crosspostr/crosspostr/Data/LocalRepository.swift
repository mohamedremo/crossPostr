...
class LocalRepository {
    ...
    func insertInSwiftData(for post: Post) throws {
        modelContainer.mainContext.insert(post)
        try modelContainer.mainContext.save()
    }
    
    func deleteFromSwiftData(for post: Post) throws {
        modelContainer.mainContext.delete(post)
        try modelContainer.mainContext.save()
    }
    ...
    func storeImageInCache(_ image: UIImage, id: UUID) throws -> URL {
        ...
        let fileName = "image-\(UUID().uuidString).jpg"  // Eindeutiger Name
        ...
        print("Image gespeichert: \(fileURL)")
        return fileURL
    }

    func storeVideoInCache(_ videoURL: URL, id: UUID) throws -> URL {
        let folderURL = try createFolder(with: id)
        let fileName = "video-\(UUID().uuidString).mp4"  // Eindeutiger Name
        ...
        print("Video gespeichert: \(destinationURL)")
...
//    }
    ...
    func deleteAllFiles(for id: UUID) throws {
        let folderURL = try createFolder(with: id)
    ...
    func retrieveFiles(for id: UUID) throws -> [URL] {
        let folderURL = try createFolder(with: id)
        return try FileManager.default.contentsOfDirectory(at: folderURL, includingPropertiesForKeys: nil)
    }
    
    func retrieveImages(for id: UUID) throws -> [URL] {
        return try retrieveFiles(for: id).filter { $0.pathExtension.lowercased() == "jpg" }
    }

    func retrieveVideos(for id: UUID) throws -> [URL] {
        return try retrieveFiles(for: id).filter { $0.pathExtension.lowercased() == "mp4" }
    }
    
    func createFolder(with id: UUID) throws -> URL {
